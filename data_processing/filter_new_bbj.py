...
result_table_path = "filtered_table_append_BBJ.txt"
list_df = pd.read_csv("SNP_name.csv")
...
def main():
    ...
    csv_file_path = os.path.join(current_folder, 'phenotypes_append.csv')
    ...
    for dir_name in next(os.walk(target_folder))[1]:
        ...
        if os.path.isdir(dir_path) and "BBJ" in dir_name:
            ...
            if disease_name in phe_names and disease_name not in result_table.columns:
                ...
                if save_counter % 10 == 0:  # Save every 10 processed diseases
                    save_path = f"filtered_table_append_BBJ_{save_counter}.txt" 
